...
df = pd.read_csv(r'C:\Users\51418\Desktop\save\S2SAN_rate-reviewText.csv')
...
for train, test in kfold.split(X, Y):
    ...
    prediected = model.predict(X)
    # prediected = model.predict(x_train, batch_size=50)
    data = pd.DataFrame(prediected)
    data.to_csv('prediection_rate-reviewText.csv')
    total_training_time = time.clock() - start

    index += 1

    with open(r"C:\Users\51418\Desktop\save\model_fitting_log", 'a') as f:
        f.write('Total training time of {}_{} is {}\n'.format(file_name, index, total_training_time))

    model.save(filepath=r"C:\Users\51418\Desktop\save\{}_{}_{}.h5".format(file_name, index, time.time()))
    history_dict = history.history
    with open(r"C:\Users\51418\Desktop\save\{}_{}_{}.dic".format(file_name, index, time.time()), "wb") as f:
